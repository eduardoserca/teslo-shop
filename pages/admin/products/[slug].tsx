...
const ProductAdminPage:FC<Props> = ({ product }) => {
    ...
    const onDeleteImage = async(id: string) => {
        ...
        setValue(
            'images',
            getValues('images').filter( img => img.id!== id ),
            {shouldValidate: true}
        );
    }
                            ...
                            <Grid container spacing={2}>
                                {
                                    getValues('images').map( img => (
                                        <Grid 
                                            ...
                                            xs={4} 
                                            sm={3} 
                                            key={img.id.includes('.jpg') ? `${img.id}_1`: img.id }
                                        >
                                            ...
                                            <Card>
                                                <CardMedia 
                                                    component='img'
                                                    className='fadeIn'
                                                    image={ img.path.includes('cloudinary.com') ? img.path : `/products/${img.path}` } 
                                                    alt={ img.path }
                                                />
                                                <CardActions>
                                                    <Button 
...
export const getServerSideProps: GetServerSideProps = async ({ query }) => {
    ...
    if( slug === 'new' ){
        ...
        tempProduct.images = [
                {id:'img1.jpg', path: 'img1.jpg'}, 
                {id:'img2.jpg', path:'img2.jpg' }
            ];
        product = tempProduct;
